...
const NetIncomeReport = ({ entries, incomes }) => {
    ...
    useEffect(() => {
        ...
        const totalExpenseAmount = entries.reduce((sum, cost) => sum + parseFloat(cost.amount || 0), 0);
        ...
        setNetIncomeUntilNow(totalIncomeAmountUntilNow - totalExpenseAmountUntilNow);
    ...
    return (
        <div style={{ display: "flex", justifyContent: "center", gap: "20px", marginTop: "20px" }}>
            ...
            <div className="card" style={{ width: "45%", padding: "20px", textAlign: "center" }}>
                <h2>Total Income & Expenses (All Time)</h2>
                <p><strong>Total Income:</strong> {totalIncome.toFixed(2)}</p>
                <p><strong>Total Expenses:</strong> {totalExpenses.toFixed(2)}</p>
                <p style={{ fontWeight: "bold", fontSize: "18px", color: netIncome >= 0 ? "green" : "red" }}>
            ...
            <div className="card" style={{ width: "45%", padding: "20px", textAlign: "center" }}>
                ...
                <p><strong>Total Income (Until Now):</strong> {totalIncomeUntilNow.toFixed(2)}</p>
                <p><strong>Total Expenses (Until Now):</strong> {totalExpensesUntilNow.toFixed(2)}</p>
                <p style={{ fontWeight: "bold", fontSize: "18px", color: netIncomeUntilNow >= 0 ? "green" : "red" }}>
                    <strong>Net Income (Until Now):</strong> {netIncomeUntilNow.toFixed(2)}
                </p>
            </div>
        </div>
    );
